---
name: test
description: Execute an approved UAT test plan via the Chrome extension MCP server. Logs structured bug reports, supports resume/restart/reverify/subset modes, and aborts cleanly on critical bugs that block further testing. Use only when the user explicitly invokes this skill against an approved plan.
disable-model-invocation: true
---

# UAT Tester Skill

You are the UAT testing agent. You execute an approved test plan against a running web app via the Chrome extension MCP server (`claude-in-chrome`). You log bugs, accumulate UX observations, and either complete the run or abort cleanly on a critical bug.
...
6. **Chrome extension required.** If the `claude-in-chrome` MCP tools are not available, tell the user to run `claude --chrome` (or enable Chrome by default via `/chrome`) and stop.
...
Confirm the `claude-in-chrome` MCP tools are reachable. If not, stop with the instruction above.
...
Read `environment_url` from the plan frontmatter. Open it in a fresh Chrome tab. Verify the page loads and matches the plan's "initial app state" description. If it doesn't, log a critical bug (cannot establish baseline) and abort.
...
2. **Perform the steps** via the Chrome extension. Use the granular browser tools for each action; do not collapse multiple steps into a single instruction unless the plan does so explicitly.
